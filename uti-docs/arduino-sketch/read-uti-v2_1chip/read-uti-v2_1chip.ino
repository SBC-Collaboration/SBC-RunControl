...
IPAddress nameserver(192,168,3,1);

//IPAddress ip(129,105,21,150);
//IPAddress gateway(129,105,21,4);
//IPAddress subnet(255,255,255,0);
//IPAddress nameserver(129,105,21,4);
...
// board connections
const uint8_t  NCHIPS = 1;    // number of chips on the board
...
const uint8_t  out[6]  = { 11, 44, 34,  7,  26, 32 };  
const uint8_t  sel2[6] = {  2, 42, 48,  5,  24, 30 };  
const uint8_t  sel3[6] = {  3, 40, 46,  6,  22, 28 };  
...
float ratio[3];   // D/C, E/C, F/C
    ...
    for(i=0; i<NCHIPS; i++) { //NCHIPS
      ...
      if (! phases) {  
         enc_float(&Mb.MbData[mbo+MBOFF_DATA], -1.);
         Serial.print("Addr: "); Serial.print(mbo+MBOFF_DATA); Serial.println(" No edge detected.");
      ...
      for (int v = 0; v < nval; v++) {
        ...
        Serial.print("Addr: "); Serial.print(mbo+MBOFF_DATA+2*v); Serial.print("; Value: "); Serial.println(r,6);
