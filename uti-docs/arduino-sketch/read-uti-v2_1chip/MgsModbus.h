...
public:
  // general
  MgsModbus(word MbDataLen, word * MbData);
  
  word *MbData;   //pointer to data block
...
private: 
  ...
  //modbus slave
  uint8_t MbsByteArray[MBAS]; // send and recieve buffer
  MB_FC MbsFC;
  word  MbDataLen; // size of data block
};
